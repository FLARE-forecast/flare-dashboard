...
df_insitu_scores <- arrow::open_dataset(s3_score, unify_schemas = TRUE) |> 
...
#Past Performance
df <- arrow::open_dataset(s3_score, unify_schemas = TRUE) |> 
  filter(variable == "temperature",
         depth %in% c(1.0),
         site_id == "fcre") |> 
  dplyr::collect() 
...
full_forecast <- arrow::open_dataset(s3_score, unify_schemas = TRUE) |> 
  filter(site_id == "fcre") |> 
...
df_insitu_forecast <- arrow::open_dataset(s3_score, unify_schemas = TRUE) |> 
  filter(variable == "temperature",
         depth == 1.0,
